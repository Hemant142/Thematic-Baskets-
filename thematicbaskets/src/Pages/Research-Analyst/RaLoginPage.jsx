...
export default function RaLoginPage() {
    ...
    dispatch(otpVarificationManager(otp, authToken))
      .then((response) => {
        ...
        if (response.data.status === "success") {
          Cookies.set(
            "login_token_ra",
            `${response.data.data.verifiedAccessToken}`
          );
          Cookies.set("username_ra", `${response.data.data.username}`);
  ...
  const handleResendOTP = () => {
    ...
    axios
      ...
      .then((otpResponse) => {
        console.log(otpResponse, "handleResendOTP");
        toast({
          title: "OTP sent to your registered email!",
          position: "bottom",
          status: "success",
        ...
        >
          ...
          >
            <Heading
              ...
              color="#244C9B"
            >
              Welcome to Research Analyst portal
            </Heading>
          </Box>
